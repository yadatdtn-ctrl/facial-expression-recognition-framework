...
Facial expression recognition (FER) is a basic problem in affective computing, with uses in healthcare (eg., mental health screening), education (student engagement), transportation (driver safety), and human computer interaction. Automatically detecting emotional states from facial images can support systems that react to a user’s feelings or help experts clarify subtle non‑verbal cues.

Convolutional Neural Networks (CNNs) are the common method for FER because they can directly learn hierarchical image features straight from raw pixel. However, a key question remains for a dataset like FER‑2013 (48×48 grayscale): is it better to create a compact CNN customized to this domain, or rely on transfer learning from larger models that have been trained on high‑resolution RGB images such as ImageNet? Reported accuracies on FER‑2013 are still far from perfect, mainly because the trade‑off between custom CNNs and transfer learning in this low‑resolution situation is not fully understood.

Transfer learning model such as MobileNetV2 and ResNet50 manage strong pretrained features but they encounter a domain mismatch: FER-2013 uses 48x48 grayscale faces with noise and class imbalance, but they anticipate 224x224 RGB input. For 48x48 grayscale, a compact CNN can be created specifically, possibly achieving good performance with fewer parameters and reducing computational cost. The dataset has challenges: Disgust only has 436 examples, whereas Happy has 7,215; labels are crowdsourced, and the occlusions and facial locations differ.
