...
Facial expression recognition (FER) is a fundamental problem in affective computing and human-computer interaction. The ability to automatically detect and classify human emotional states from facial images has significant practical value across a range of applied domains, including adaptive educational systems, healthcare monitoring, driver safety analysis, and human-robot interaction. [EXPAND WITH 2–3 CITED SOURCES FROM PERPLEXITY]

Convolutional Neural Networks (CNNs) have emerged as the dominant paradigm for image-based expression recognition due to their capacity to automatically learn hierarchical spatial features directly from pixel data, eliminating the need for handcrafted feature extraction. [EXPAND]

Despite strong benchmark performance, a critical limitation of many deployed CNN models is the opacity of their decision-making process. In human-centered applications such as FER, understanding *why* a model assigns a particular expression label is as important as the accuracy of that assignment. Explainable AI methods, specifically Gradient-weighted Class Activation Mapping (Grad-CAM) and SHapley Additive exPlanations (SHAP), provide complementary tools for visualizing and quantifying model attention at the spatial and pixel level respectively. [EXPAND]

This project is motivated by the need to develop a rigorous, reproducible, and explainable benchmarking framework for facial expression recognition that bridges raw classification performance and model interpretability.
